...
import type { Organisationseinheiten, SoaManagement, KontrollManagement, AufgabenFreigaben, AwarenessSchulungen, PolicyManagement, FindingsAbweichungen, FrameworkVerwaltung, IncidentManagement, MassnahmenManagement, DokumenteEvidenzen, AuditManagement, AssetRegister, BcmNotfallmanagement, Lieferantenmanagement, RisikoRegister, CreateOrganisationseinheiten, CreateSoaManagement, CreateKontrollManagement, CreateAufgabenFreigaben, CreateAwarenessSchulungen, CreatePolicyManagement, CreateFindingsAbweichungen, CreateFrameworkVerwaltung, CreateIncidentManagement, CreateMassnahmenManagement, CreateDokumenteEvidenzen, CreateAuditManagement, CreateAssetRegister, CreateBcmNotfallmanagement, CreateLieferantenmanagement, CreateRisikoRegister } from '@/types/app';
...
export class LivingAppsService {
  ...
  // --- RISIKO_REGISTER ---
  static async getRisikoRegister(): Promise<RisikoRegister[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.RISIKO_REGISTER}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as RisikoRegister[];
    return enrichLookupFields(records, 'risiko_register');
  }
  static async getRisikoRegisterEntry(id: string): Promise<RisikoRegister | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.RISIKO_REGISTER}/records/${id}`);
    const record = { record_id: data.id, ...data } as RisikoRegister;
    return enrichLookupFields([record], 'risiko_register')[0];
  }
  static async createRisikoRegisterEntry(fields: CreateRisikoRegister) {
    return callApi('POST', `/apps/${APP_IDS.RISIKO_REGISTER}/records`, { fields: cleanFieldsForApi(fields as any, 'risiko_register') });
  }
  static async updateRisikoRegisterEntry(id: string, fields: Partial<CreateRisikoRegister>) {
    return callApi('PATCH', `/apps/${APP_IDS.RISIKO_REGISTER}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'risiko_register') });
  }
  static async deleteRisikoRegisterEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.RISIKO_REGISTER}/records/${id}`);
  }
