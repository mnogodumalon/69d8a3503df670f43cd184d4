...
interface OrganisationseinheitenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Organisationseinheiten['fields']) => Promise<void>;
  defaultValues?: Organisationseinheiten['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function OrganisationseinheitenDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: OrganisationseinheitenDialogProps) {
  const [fields, setFields] = useState<Partial<Organisationseinheiten['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'organisationseinheiten');
      await onSubmit(clean as Organisationseinheiten['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "org_housenumber": string | null, // Hausnummer\n  "org_postal": string | null, // Postleitzahl\n  "org_city": string | null, // Stadt\n  "org_country": string | null, // Land\n  "org_responsible_firstname": string | null, // Verantwortlicher Vorname\n  "org_responsible_lastname": string | null, // Verantwortlicher Nachname\n  "org_responsible_email": string | null, // E-Mail Verantwortlicher\n  "org_description": string | null, // Beschreibung\n  "org_active": boolean | null, // Aktiv\n  "org_name": string | null, // Name der Organisationseinheit\n  "org_type": LookupValue | null, // Typ (select one key: "abteilung" | "bereich" | "standort" | "tochtergesellschaft" | "konzerngesellschaft") mapping: abteilung=Abteilung, bereich=Bereich, standort=Standort, tochtergesellschaft=Tochtergesellschaft, konzerngesellschaft=Konzerngesellschaft\n  "org_parent": string | null, // Übergeordnete Einheit\n  "org_street": string | null, // Straße\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Organisationseinheiten['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Organisationseinheiten bearbeiten' : 'Organisationseinheiten hinzufügen';
