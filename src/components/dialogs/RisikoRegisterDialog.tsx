...
interface RisikoRegisterDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: RisikoRegister['fields']) => Promise<void>;
  defaultValues?: RisikoRegister['fields'];
...
export function RisikoRegisterDialog({ open, onClose, onSubmit, defaultValues, asset_registerList, organisationseinheitenList, enablePhotoScan = true, enablePhotoLocation = true }: RisikoRegisterDialogProps) {
  const [fields, setFields] = useState<Partial<RisikoRegister['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'risiko_register');
      await onSubmit(clean as RisikoRegister['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<RisikoRegister['fields']>;
      });
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Risiko-Register bearbeiten' : 'Risiko-Register hinzufügen';
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="risk_id">Risiko-ID</Label>
            <Input
              id="risk_id"
              value={fields.risk_id ?? ''}
              onChange={e => setFields(f => ({ ...f, risk_id: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <Select
              value={lookupKey(fields.risk_category) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, risk_category: v === 'none' ? undefined : v as any }))}
            >
              <SelectTrigger id="risk_category"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
                <SelectItem value="infosec">Informationssicherheit</SelectItem>
                <SelectItem value="datenschutz">Datenschutz</SelectItem>
                <SelectItem value="betrieb">Betriebsrisiko</SelectItem>
                <SelectItem value="compliance">Compliance</SelectItem>
                <SelectItem value="drittpartei">Drittpartei / Lieferant</SelectItem>
                <SelectItem value="physisch">Physische Sicherheit</SelectItem>
                <SelectItem value="personal">Personalrisiko</SelectItem>
          ...
          <div className="space-y-2">
            <Label htmlFor="risk_org_unit">Betroffene Organisationseinheit</Label>
            <Select
