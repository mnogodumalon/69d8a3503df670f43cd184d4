...
interface FrameworkVerwaltungDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: FrameworkVerwaltung['fields']) => Promise<void>;
  defaultValues?: FrameworkVerwaltung['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function FrameworkVerwaltungDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: FrameworkVerwaltungDialogProps) {
  const [fields, setFields] = useState<Partial<FrameworkVerwaltung['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'framework_verwaltung');
      await onSubmit(clean as FrameworkVerwaltung['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "fw_name": string | null, // Framework-Name\n  "fw_type": LookupValue | null, // Framework-Typ (select one key: "iso27001" | "nis2" | "dora" | "bsi" | "soc2" | "sonstiges") mapping: iso27001=ISO/IEC 27001, nis2=NIS2, dora=DORA, bsi=BSI IT-Grundschutz, soc2=SOC 2, sonstiges=Sonstiges\n  "fw_version": string | null, // Version / Jahr\n  "fw_description": string | null, // Beschreibung\n  "req_id": string | null, // Anforderungs-ID\n  "req_title": string | null, // Anforderungstitel\n  "req_description": string | null, // Anforderungstext\n  "req_domain": string | null, // Domäne / Kapitel\n  "req_mandatory": boolean | null, // Verpflichtend\n  "fw_active": boolean | null, // Framework aktiv\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<FrameworkVerwaltung['fields']>;
      });
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Framework-Verwaltung bearbeiten' : 'Framework-Verwaltung hinzufügen';
