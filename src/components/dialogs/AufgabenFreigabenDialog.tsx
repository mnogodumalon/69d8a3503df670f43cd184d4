...
import type { AufgabenFreigaben, RisikoRegister, MassnahmenManagement, AuditManagement } from '@/types/app';
...
export function AufgabenFreigabenDialog({ open, onClose, onSubmit, defaultValues, risiko_registerList, maßnahmen_managementList, audit_managementList, enablePhotoScan = true, enablePhotoLocation = true }: AufgabenFreigabenDialogProps) {
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="task_related_risk" entity="Risiko-Register">\n${JSON.stringify(risiko_registerList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      ...
      setFields(prev => {
        ...
        if (task_related_riskName) {
          const task_related_riskMatch = risiko_registerList.find(r => matchName(task_related_riskName!, [String(r.fields.risk_id ?? '')]));
          if (task_related_riskMatch) merged['task_related_risk'] = createRecordUrl(APP_IDS.RISIKO_REGISTER, task_related_riskMatch.record_id);
        }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={v => setFields(f => ({ ...f, task_related_risk: v === 'none' ? undefined : createRecordUrl(APP_IDS.RISIKO_REGISTER, v) }))}
            >
