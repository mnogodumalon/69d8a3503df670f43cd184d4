...
interface IncidentManagementDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: IncidentManagement['fields']) => Promise<void>;
  defaultValues?: IncidentManagement['fields'];
...
export function IncidentManagementDialog({ open, onClose, onSubmit, defaultValues, asset_registerList, organisationseinheitenList, enablePhotoScan = true, enablePhotoLocation = true }: IncidentManagementDialogProps) {
  const [fields, setFields] = useState<Partial<IncidentManagement['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'incident_management');
      await onSubmit(clean as IncidentManagement['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="incident_affected_asset" entity="Asset-Register">\n${JSON.stringify(asset_registerList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      contextParts.push(`<available-records field="incident_affected_org" entity="Organisationseinheiten">\n${JSON.stringify(organisationseinheitenList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      const schema = `{\n  "incident_id": string | null, // Vorfalls-ID\n  "incident_title": string | null, // Vorfallsbezeichnung\n  "incident_description": string | null, // Vorfallsbeschreibung\n  "incident_category": LookupValue | null, // Vorfallskategorie (select one key: "malware" | "phishing" | "datenpanne" | "unbefugter_zugriff" | "dos" | "physisch" | "systemausfall" | "insider" | "sonstiges") mapping: malware=Malware / Ransomware, phishing=Phishing / Social Engineering, datenpanne=Datenpanne / Datenleck, unbefugter_zugriff=Unbefugter Zugriff, dos=Denial of Service, physisch=Physischer Einbruch, systemausfall=Systemausfall, insider=Insider-Bedrohung, sonstiges=Sonstiges\n  "incident_severity": LookupValue | null, // Schweregrad (select one key: "kritisch" | "hoch" | "mittel" | "niedrig") mapping: kritisch=Kritisch, hoch=Hoch, mittel=Mittel, niedrig=Niedrig\n  "incident_detected_at": string | null, // YYYY-MM-DDTHH:MM\n  "incident_occurred_at": string | null, // YYYY-MM-DDTHH:MM\n  "incident_reporter_firstname": string | null, // Melder Vorname\n  "incident_reporter_lastname": string | null, // Melder Nachname\n  "incident_reporter_email": string | null, // Melder E-Mail\n  "incident_affected_asset": string | null, // Display name from Asset-Register (see <available-records>)\n  "incident_affected_org": string | null, // Display name from Organisationseinheiten (see <available-records>)\n  "incident_nis2_reportable": boolean | null, // NIS2-meldepflichtig\n  "incident_dora_reportable": boolean | null, // DORA-meldepflichtig\n  "incident_status": LookupValue | null, // Status (select one key: "neu" | "in_bearbeitung" | "eskaliert" | "behoben" | "geschlossen") mapping: neu=Neu gemeldet, in_bearbeitung=In Bearbeitung, eskaliert=Eskaliert, behoben=Behoben, geschlossen=Geschlossen\n  "incident_notes": string | null, // Weitere Informationen\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["incident_affected_asset", "incident_affected_org"]);
        for (const [k, v] of Object.entries(raw)) {
          if (applookupKeys.has(k)) continue;
          if (v != null) merged[k] = v;
        }
        const incident_affected_assetName = raw['incident_affected_asset'] as string | null;
        if (incident_affected_assetName) {
          const incident_affected_assetMatch = asset_registerList.find(r => matchName(incident_affected_assetName!, [String(r.fields.asset_name ?? '')]));
          if (incident_affected_assetMatch) merged['incident_affected_asset'] = createRecordUrl(APP_IDS.ASSET_REGISTER, incident_affected_assetMatch.record_id);
        }
        const incident_affected_orgName = raw['incident_affected_org'] as string | null;
        if (incident_affected_orgName) {
          const incident_affected_orgMatch = organisationseinheitenList.find(r => matchName(incident_affected_orgName!, [String(r.fields.org_housenumber ?? '')]));
          if (incident_affected_orgMatch) merged['incident_affected_org'] = createRecordUrl(APP_IDS.ORGANISATIONSEINHEITEN, incident_affected_orgMatch.record_id);
        }
        return merged as Partial<IncidentManagement['fields']>;
      });
      ...
      if (file && (file.type.startsWith('image/') || file.type === 'application/pdf')) {
        try {
          ...
          setFields(prev => ({ ...prev, incident_evidence: fileUrl }));
        } catch (uploadErr) {
  ...
  const DIALOG_INTENT = defaultValues ? 'Incident-Management bearbeiten' : 'Incident-Management hinzufügen';
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="incident_affected_asset">Betroffenes Asset</Label>
            <Select
              value={extractRecordId(fields.incident_affected_asset) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, incident_affected_asset: v === 'none' ? undefined : createRecordUrl(APP_IDS.ASSET_REGISTER, v) }))}
            >
              <SelectTrigger id="incident_affected_asset"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
          ...
          <div className="space-y-2">
            <Label htmlFor="incident_affected_org">Betroffene Organisationseinheit</Label>
            <Select
              value={extractRecordId(fields.incident_affected_org) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, incident_affected_org: v === 'none' ? undefined : createRecordUrl(APP_IDS.ORGANISATIONSEINHEITEN, v) }))}
            >
              <SelectTrigger id="incident_affected_org"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
