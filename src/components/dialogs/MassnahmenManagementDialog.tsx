...
export function MassnahmenManagementDialog({ open, onClose, onSubmit, defaultValues, risikomanagementList, enablePhotoScan = true, enablePhotoLocation = true }: MassnahmenManagementDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        if (measure_riskName) {
          const measure_riskMatch = risikomanagementList.find(r => matchName(measure_riskName!, [String(r.fields.risk_owner_firstname ?? '')]));
          if (measure_riskMatch) merged['measure_risk'] = createRecordUrl(APP_IDS.RISIKOMANAGEMENT, measure_riskMatch.record_id);
        }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={v => setFields(f => ({ ...f, measure_risk: v === 'none' ? undefined : createRecordUrl(APP_IDS.RISIKOMANAGEMENT, v) }))}
            >
              ...
              <SelectContent>
                ...
                {risikomanagementList.map(r => (
                  <SelectItem key={r.record_id} value={r.record_id}>
                    {r.fields.risk_owner_firstname ?? r.record_id}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
