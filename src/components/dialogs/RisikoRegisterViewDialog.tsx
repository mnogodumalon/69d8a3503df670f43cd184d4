...
interface RisikoRegisterViewDialogProps {
  open: boolean;
  onClose: () => void;
  record: RisikoRegister | null;
  onEdit: (record: RisikoRegister) => void;
  asset_registerList: AssetRegister[];
  organisationseinheitenList: Organisationseinheiten[];
}

export function RisikoRegisterViewDialog({ open, onClose, record, onEdit, asset_registerList, organisationseinheitenList }: RisikoRegisterViewDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Risiko-Register anzeigen</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Risiko-ID</Label>
            <p className="text-sm">{record.fields.risk_id ?? '—'}</p>
          </div>
          <div className="space-y-1">
          ...
          <div className="space-y-1">
            ...
            <Badge variant="secondary">{record.fields.risk_category?.label ?? '—'}</Badge>
          </div>
          <div className="space-y-1">
