...
export default function MassnahmenManagementPage() {
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [mainData, risiko_registerData] = await Promise.all([
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{getRisikoRegisterDisplayName(record.fields.measure_risk)}</span></TableCell>
