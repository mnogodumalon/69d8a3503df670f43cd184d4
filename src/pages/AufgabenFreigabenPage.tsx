...
export default function AufgabenFreigabenPage() {
  ...
  const [risiko_registerList, setRisikoRegisterList] = useState<RisikoRegister[]>([]);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [mainData, risiko_registerData, maßnahmen_managementData, audit_managementData] = await Promise.all([
      ...
      setRecords(mainData);
      setRisikoRegisterList(risiko_registerData);
  ...
  return (
    ...
    >
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{getRisikoRegisterDisplayName(record.fields.task_related_risk)}</span></TableCell>
      ...
      <AufgabenFreigabenDialog
        ...
        risiko_registerList={risiko_registerList}
