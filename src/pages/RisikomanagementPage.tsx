...
export default function RisikomanagementPage() {
  const [records, setRecords] = useState<Risikomanagement[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Risikomanagement | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Risikomanagement | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Risikomanagement | null>(null);
  ...
  async function handleCreate(fields: Risikomanagement['fields']) {
    await LivingAppsService.createRisikomanagementEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Risikomanagement['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateRisikomanagementEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteRisikomanagementEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} Risikomanagement im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Risikomanagement suchen..."
          value={search}
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell className="font-medium">{record.fields.risk_owner_firstname ?? '—'}</TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={19} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Risikomanagement. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      <ConfirmDialog
        ...
        title="Risikomanagement löschen"
